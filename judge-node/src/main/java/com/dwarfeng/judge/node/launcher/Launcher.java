...
public class Launcher {
    ...
    private static void mayResetSinker(ApplicationContext ctx) {
        // 获取启动器设置处理器，用于获取启动器设置，并按照设置选择性执行功能。
        LauncherSettingHandler launcherSettingHandler = ctx.getBean(LauncherSettingHandler.class);

        // 如果不重置下沉器，则返回。
        if (!launcherSettingHandler.isResetSinkerSupport()) {
            return;
        }
    ...
    private static void mayResetAdapter(ApplicationContext ctx) {
        // 获取启动器设置处理器，用于获取启动器设置，并按照设置选择性执行功能。
        LauncherSettingHandler launcherSettingHandler = ctx.getBean(LauncherSettingHandler.class);

        // 如果不重置适配器，则返回。
        if (!launcherSettingHandler.isResetAdapterSupport()) {
            return;
        }
