...
public class CronDriverProvider implements DriverProvider {
    ...
    public static class CronDriver extends AbstractDriver {
        ...
        public void register(DriverInfo driverInfo) throws DriverException {
            ...
            try {
                ...
                CronProcessor cronProcessor = new CronProcessor(context, sectionKey);
    ...
    private static class CronProcessor implements Runnable {
        ...
        private CronProcessor(Driver.Context context, LongIdKey sectionKey) {
        ...
        public void run() {
            ...
            try {
                ...
                LOGGER.debug("计划时间已到达, cron 驱动器驱动部件 {} 执行统计动作...", sectionKey);
                context.execute(sectionKey);
            } catch (Exception e) {
                LOGGER.warn("部件 {} 执行统计动作时出现异常, 放弃本次统计", sectionKey, e);
            } finally {
                lock.unlock();
            }
        }
