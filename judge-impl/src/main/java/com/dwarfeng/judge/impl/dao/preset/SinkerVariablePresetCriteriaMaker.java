...
@Component
public class SinkerVariablePresetCriteriaMaker implements PresetCriteriaMaker {

    @SuppressWarnings("SwitchStatementWithTooFewBranches")
    @Override
    public void makeCriteria(DetachedCriteria criteria, String preset, Object[] objs) {
        switch (preset) {
            case SinkerVariableMaintainService.CHILD_FOR_SINKER_INFO:
    ...
    private void childForSinkerInfo(DetachedCriteria criteria, Object[] objs) {
        try {
            if (Objects.isNull(objs[0])) {
                ...
                LongIdKey sinkerInfoKey = (LongIdKey) objs[0];
                criteria.add(Restrictions.eqOrIsNull("sinkerLongId", sinkerInfoKey.getLongId()));
            }
        } catch (Exception e) {
