...
@TelqosCommand
public class SectionBindingLocalCacheCommand extends CliCommand {

    private static final Logger LOGGER = LoggerFactory.getLogger(SectionBindingLocalCacheCommand.class);
    ...
    private static final String IDENTITY = "scblc";
    private static final String DESCRIPTION = "部件绑定本地缓存操作";

    private static final String CMD_LINE_SYNTAX_LOOKUP = IDENTITY + " " +
            CommandUtil.concatOptionPrefix(COMMAND_OPTION_LOOKUP) + " section-info-id";
    ...
    private final SectionBindingQosService sectionBindingQosService;

    public SectionBindingLocalCacheCommand(SectionBindingQosService sectionBindingQosService) {
        super(IDENTITY, DESCRIPTION, CMD_LINE_SYNTAX);
        this.sectionBindingQosService = sectionBindingQosService;
    }

    @Override
    protected List<Option> buildOptions() {
        List<Option> list = new ArrayList<>();
        list.add(Option.builder(COMMAND_OPTION_LOOKUP).desc("查询部件绑定").hasArg().type(Number.class).build());
        list.add(Option.builder(COMMAND_OPTION_CLEAR).desc("清除部件绑定本地缓存").build());
        return list;
    }
    ...
    private void handleLookup(Context context, CommandLine cmd) throws Exception {
        ...
        SectionBinding sectionBinding = sectionBindingQosService.getSectionBinding(new LongIdKey(sectionId));
        if (Objects.isNull(sectionBinding)) {
            context.sendMessage("not exists!");
            return;
        }
        List<Map.Entry<LongIdKey, Map<String, SinkerMetaInfo>>> sinkerMetaInfoMapMapEntries =
                sectionBinding.getMap().entrySet()
                        .stream()
    ...
    private void printSinkerMetaInfoMapMapEntry(
        ...
        if (Objects.isNull(entry)) {
            ...
            LongIdKey sinkerKey = entry.getKey();
            Map<String, SinkerMetaInfo> sinkerMetaInfoMap = entry.getValue();
            context.sendMessage(String.format("sinkerKey: %s", sinkerKey));
    ...
    private void handleClear(Context context) throws Exception {
        sectionBindingQosService.clearLocalCache();
